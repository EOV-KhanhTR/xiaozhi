...
static void EyeRenderer_drawFrame(int16_t topOffset, float scale, uint8_t blinkMask) {
  ...
  // Check if Doraemon face is active and render it instead
  if (DoraemonFace::isActive()) {
    DoraemonFace::Expression expr = DoraemonFace::NORMAL;
    if (emotionState.currentEmotion == EYE_EMO_HAPPY1 || emotionState.currentEmotion == EYE_EMO_HAPPY2 || emotionState.happyActive) {
      expr = DoraemonFace::HAPPY;
    } else if (emotionState.currentEmotion == EYE_EMO_SAD1 || emotionState.currentEmotion == EYE_EMO_SAD2) {
      expr = DoraemonFace::CRYING;
    } else if (emotionState.currentEmotion == EYE_EMO_LOVE || emotionState.currentEmotion == EYE_EMO_EXCITED || emotionState.excitedActive) {
      expr = DoraemonFace::SMILING;
    }
    DoraemonFace::setExpression(expr);
    DoraemonFace::update(millis());
    bool stillActive = DoraemonFace::renderFrame(eyeCanvasActive, millis());
    if (stillActive) {
...
void DisplaySystem_begin() {
  ...
  DoraemonFace::begin();
  DoraemonFace::activate();  // Start Doraemon face on boot
...
void DisplaySystem_update() {
  ...
  if (touchState.blockGesturesUntilLift || touchState.suppressMenuOpenUntilLift) {
    ...
    // Forward touch to DoraemonFace if active
    if (DoraemonFace::isActive() && touch.gesture == TOUCH_TAP) {
      DoraemonFace::handleTouch(touch.x, touch.y);
    }
